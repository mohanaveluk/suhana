...
@Component({
  ...
  imports: [ReactiveFormsModule, MaterialModule, TitleCasePipe],
...
export class AdminEditProfileComponent implements OnInit {
  ...
  protected async onFileSelected(event: Event): Promise<void> {
    ...
    try {
      const f = this.avatarFile();
      if (f) {
        const url = await this.profileService.uploadAdmxPhoto(this.profileId()!, f);
        if (url) {
          this.currentAvatarUrl.set(url);
          const p = this.avatarPreview();
  ...
  protected readonly profileStatusOptions: { value: ProfileStatus; label: string }[] = [
    ...
    { value: 'blocked',  label: 'Blocked — access restricted' },
  ];
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      const profile = await firstValueFrom(this.api.getProfileByIdAdmx(userId));
      if (profile) this.patchForms(profile);
    } catch {
      this.saveError.set('Failed to load profile. Please go back and try again.');
    } finally {
  ...
  private patchForms(p: UserProfile): void {
    ...
    this.profileId.set(p.user !== undefined ? p.user.id : null);
  ...
  async saveAll(): Promise<void> {
    ...
    if (!userId) {
      this.saveError.set('No profile ID found. Cannot save.');
      this.isSaving.set(false);
      return;
    }
    ...
    try {
      await firstValueFrom(this.api.updateNewProfileAdmx(userId, updated));
  ...
  protected goBack(): void { this.router.navigate(['/admin']); }
  ...
  private async processHoroscopeDoc(file: File): Promise<void> {
    ...
    try {
      const res = await firstValueFrom(this.api.uploadHoroscopeDocAdmx(this.profileId()!, file));
