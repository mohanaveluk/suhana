export interface Country {
  name: string;
  /** ISO 3166-1 alpha-2, lowercased for the flag CDN path. */
  iso2: string;
  /** E.164 calling code, including the leading "+". */
  dialCode: string;
  /** Wins the tie when several countries share a dial code (e.g. +1, +7). */
  primary?: boolean;
}
...
export const COUNTRIES: Country[] = [
  ...
  { name: 'Central African Rep.',   iso2: 'CF', dialCode: '+236' },
  ...
  { name: 'Russia',                 iso2: 'RU', dialCode: '+7', primary: true },
  ...
  { name: 'United States',          iso2: 'US', dialCode: '+1', primary: true },
...
export const DEFAULT_COUNTRY: Country = BY_ISO.get(DEFAULT_COUNTRY_ISO)!;

export function findCountry(iso2: string | null | undefined): Country {
  return (iso2 ? BY_ISO.get(iso2) : undefined) ?? DEFAULT_COUNTRY;
}

/** 40px-wide PNG, rendered at 20px so it stays sharp on retina displays. */
export function flagUrl(iso2: string): string {
  return `https://flagcdn.com/w40/${iso2.toLowerCase()}.png`;
}

/**
 * Splits a stored number into country + national part. Matches the longest
 * dial code so "+1868…" resolves to Trinidad rather than the US, and falls
 * back to the default country when there is no leading "+".
 */
export function splitDialCode(raw: string | null | undefined): { iso2: string; national: string } {
  const v = String(raw ?? '').replace(/[\s\-().]/g, '');

  if (v.startsWith('+')) {
    let best: Country | undefined;
    for (const c of COUNTRIES) {
      if (!v.startsWith(c.dialCode)) continue;
      const longer = !best || c.dialCode.length > best.dialCode.length;
      const tieBreak = best && c.dialCode.length === best.dialCode.length && c.primary;
      if (longer || tieBreak) best = c;
    }
    if (best) {
      return { iso2: best.iso2, national: v.slice(best.dialCode.length).replace(/\D/g, '') };
    }
  }

  return { iso2: DEFAULT_COUNTRY_ISO, national: v.replace(/\D/g, '') };
}
